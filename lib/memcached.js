...
Client.config = {
  ...
  , serializer: false       // function for serialize values
  , unserializer: false     // function for deserialize serialized data
};
  ...
  , 'VALUE': function value(tokens, dataSet, err, queue) {
      ...
      switch (flag) {
        case FLAG_JSON:
          ...
          break;
        case FLAG_SERIALIZED:
          if (typeof this.memcached.unserializer === 'function') {
              dataSet = this.memcached.unserializer(dataSet);
          }
          break;
        case FLAG_NUMERIC:
          dataSet = +dataSet;
